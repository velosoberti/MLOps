...
class PredictionClient:
    ...
    def predict_single(self, data: dict[str, Any], verbose: bool = True) -> dict | None:
        """
        ...
        try:
            response = requests.post(
                f"{self.base_url}/predict", json=data, headers={"Content-Type": "application/json"}
            )
    ...
    def predict_batch(self, instances: list, verbose: bool = True) -> dict | None:
        """
        ...
        if status_code == 200:
            ...
            if result["prediction"] == "diabetes":
                ...
                if result["confidence"] > 0.8:
                    print("   Confiança ALTA - Recomendado consultar médico")
                elif result["confidence"] > 0.6:
                    print("   Confiança MÉDIA - Monitoramento recomendado")
                else:
                    print("   Confiança BAIXA - Resultado incerto")
            else:
                print(f"   Baixa probabilidade de diabetes ({result['score']:.1%})")
                if result["confidence"] > 0.8:
                    print("   Confiança ALTA - Indicadores normais")
                elif result["confidence"] > 0.6:
                    print("   Confiança MÉDIA - Manter hábitos saudáveis")
                else:
                    print("   Confiança BAIXA - Monitoramento preventivo")
        else:
